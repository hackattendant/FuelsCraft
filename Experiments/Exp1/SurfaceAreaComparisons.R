...
for (n in 0:74) {
  # build file path
  file_to_read <- "subset"
  file_to_read <- (paste(file_to_read, toString(n), sep=""))
  file_to_read <- (paste(file_to_read, ".xyz", sep=""))
  #print(file_to_read)
  ...
  # run kde
  dens <- kde3d(data[,1], data[,2], data[,3], n=20)
  ...
  total_surface_area <- (total_surface_area + surfArea)
...
cat("\nTotal Surface Area Using subsets: ", total_surface_area)
cat("\n\n")



# _______________ calculate surface area of entire tree tip
data = read.csv("~/Desktop/FuelsCraft/R_Meshing_Playground/tipOfTree.txt")
# strip out xyz
data <- data[,1:3]
# normalize x and y channels
#data[,1] <- (data[,1]-min(data[,1]))
